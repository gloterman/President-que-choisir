...
import { clsx, type Tone } from '@/lib/format'
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3 border-b border-line px-4 py-3.5 sm:px-5">
      <div className="min-w-0">
        <Heading className="text-[0.95rem] font-semibold tracking-tight text-ink">{title}</Heading>
        {subtitle && <p className="mt-0.5 text-[0.82rem] leading-snug text-ink-2">{subtitle}</p>}
      </div>
...
export function Button({
  ...
  variant?: 'primaire' | 'secondaire' | 'discret' | 'danger'
  size?: 'normale' | 'petite'
}) {
  const variants = {
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        size === 'petite' ? 'px-2.5 py-1.5 text-[0.78rem]' : 'px-3.5 py-2 text-[0.85rem]',
        variants[variant],
        className,
      )}
      {...props}
...
export function Notice({
  ...
  return (
    <div className={clsx('rounded-xl border px-4 py-3', TONES[tone])}>
...
export function KeyValue({
  ...
  return (
    <div>
      ...
      <p className="mt-0.5 text-2xl font-semibold leading-none tracking-tight text-ink">{value}</p>
